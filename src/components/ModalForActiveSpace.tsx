...
export default function ModalForActiveSpace({ space, openModal, handleOnClose }: any) {
    ...
    return (
        <>
            <Modal className='fade-in' dismissible show={openModal} size="2xl" popup onClose={handleOnClose} >
                {/* <Modal.Header /> */}
                <Modal.Body className={`mx-0 px-0 flex h-full flex-col justify-between  animate-fade-in relative border  bg-gray-100 dark:bg-gray-800 lg:w-full ${space.users.length == 0 ? "h-60" : "h-auto"} lg:h-80 overflow-hidden ml-0 rounded-lg  shadow-lg dark:border-gray-700 `}>
                    ...
                    <div className="flex flex-wrap px-4 py-3 ">
                        ...
                        {space.users?.length && space.users.length > 9 ?
                            <div className=" h-10 w-10 rounded-full ring-2 ring-white flex items-center justify-center dark:ring-[#272F34] bg-slate-100 dark:bg-gray-600  dark:text-gray-200" style={{ fontSize: '0.8rem' }}>
                                {space.users.length}
                            </div> : ""
                        }
                    </div>
